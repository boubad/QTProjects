...
namespace info {
    ///////////////////////////////////
    static const int st_conso_cols = 7;
    static const int st_conso_rows = 12;
    ...
    static const int st_mortal_cols = 6;
    static const int st_mortal_rows = 19;
    ...
    ///////////////////////////////////
    static const int TEST_NROWS = 2048;
    static const int TEST_NCOLS = 20;
    static const char *st_test_name = "info_test";
    ///////////////////////////////////////
    void InfoTestData::get_test_indiv_data(QString &name, int &nRows, int &nCols) {
        name = st_test_name;
        nRows = TEST_NROWS;
        nCols = TEST_NCOLS;
    }
     ///////////////////////////////////////
    void InfoTestData::get_data(const QString &name, int &nRows, int &nCols,
    ...
    void InfoTestData::get_data_names(QVector<QString> &onames) {
        onames.clear();
        onames.push_back(st_conso_name);
        onames.push_back(st_mortal_name);
    }// get_data_names
    void InfoTestData::get_mortal_name(QString &name) {
        name = st_mortal_name;
    }
    void InfoTestData::get_conso_name(QString &name) {
        name = st_conso_name;
    }
    void InfoTestData::get_default_type_genre_status(QString &type, QString &genre, QString &status) {
        type = st_type;
        genre = st_genre;
        status = st_status;
    }//get_default_type_genre_status
    void InfoTestData::get_database_filename(QString &filename) {
        filename = st_database_filename;
    }// get_database_filename
     ///////////////////////////////////////
    void InfoTestData::get_mortal_data(QString &name, int &nRows, int &nCols,
        QVector<int> &data, QVector<QString> &indNames, QVector<QString> &varNames) {
        name = st_mortal_name;
        ...
        Q_ASSERT(nCols > 1);
        const int n = (int)(nCols * nRows);
        data.resize(n);
        Q_ASSERT(data.size() == n);
        indNames.resize(nRows);
        Q_ASSERT(indNames.size() == nRows);
        varNames.resize(nCols);
        Q_ASSERT(varNames.size() == nCols);
        ...
        for (int i = 0; i < nRows; ++i) {
            ...
            for (int j = 0; j < nCols; ++j) {
                const int pos = (int)(i * nCols + j);
                data[pos] = pData[pos];
            }// j
        }// i
    }//get_conso_data
    void InfoTestData::get_conso_data(QString &name, int &nRows, int &nCols,
        QVector<int> &data, QVector<QString> &indNames, QVector<QString> &varNames) {
        name = st_conso_name;
        nRows = st_conso_rows;
        Q_ASSERT(nRows > 1);
